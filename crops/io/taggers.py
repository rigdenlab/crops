    ...
    # PDBe
        ...
        for i in range(5,len(seqheader)):
            if seqheader[i]=='|':
                ...
                tag='seqid'
                newchid=''
            elif seqheader[i]==' ' or seqheader[i]==',' or i==len(seqheader)-1:
...
def retrieve_id_old(seqheader,extrainfo=False):
    ...
    namechar=False
    idchar=False
    nameseq=["", [], None] #PDB ID, chain IDs for seqgroup, seqgroup
    newchid=''
    if seqheader.startswith('>sp|'):
        for i in range(4,len(seqheader)):
            if seqheader[i]=='|':
...
def makeheader(mainid=None, seqid=None, chains=None, source=None, extrainfo=None,
    ...
    if chains is not None:
        if not isinstance(chains, set):
                raise ValueError("Argument 'chains' is not a list of strings.")
        else:
